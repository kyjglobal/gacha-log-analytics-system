...
계정 관리 화면은 현재 세션의 사용자 정보를 사용하며, 닉네임 변경 성공 시 Riverpod 세션을 즉시 갱신합니다. 회원 탈퇴가 완료되면 저장된 JWT를 제거하고 로그인 화면으로 이동합니다.
...
1. 현재 비밀번호를 다시 검증합니다.
2. 관리자 계정의 탈퇴 요청을 거부합니다.
3. 이메일과 닉네임을 사용자 ID 기반 값으로 익명화합니다.
4. 상태를 `blocked`, `is_deleted`를 `true`로 변경하고 삭제 시각을 기록합니다.
5. 기존 JWT는 이후 인증 dependency에서 거부되며 관련 로그는 FK와 사용자 ID를 유지합니다.
